...
public class BookServiceImpl implements BookService {
    ...
    @Override
    @Transactional(readOnly = true)
    public Optional<Book> getByTitleAndAuthorAndGenre(Book book) {
        try {
            return bookRepository.findByTitleAndAuthorAndGenre_Type(book.getTitle(), book.getAuthor().getName(), book.getAuthor().getSurname(), book.getGenre().getType());
        } catch (Exception ex) {
            log.error("Error with finding book by by author, title and genre {}, {}" + book.getTitle(), ex.getCause());
            throw new ServiceException("Error with finding book by author and title" + book.getTitle(), ex);
        }
    }
